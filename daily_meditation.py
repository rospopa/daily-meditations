    ...
    steps:
      ...
      - name: Run daily meditation script
        ...
        run: |
          ...
          echo "DEBUG: GVOICE_EMAIL length: ${#GVOICE_EMAIL} chars"
          echo "DEBUG: GVOICE_NUMBER: $GVOICE_NUMBER"
          echo "DEBUG: RECIPIENTS: $RECIPIENTS"
          
          # Create config.json using the setup script
          mkdir -p scripts
          python3 scripts/setup_config.py || (echo "Failed to create valid config.json" && exit 1)
